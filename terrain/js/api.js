// DarAmanah Terrain — API fetch wrapper (same pattern as antenne SPA)
const API_BASE = 'https://api.daramanah.family';

function getTokens() {
  return { access: localStorage.getItem('da_terrain_access'), refresh: localStorage.getItem('da_terrain_refresh') };
}
function setTokens(access, refresh) {
  if (access) localStorage.setItem('da_terrain_access', access);
  if (refresh) localStorage.setItem('da_terrain_refresh', refresh);
}
function clearTokens() {
  localStorage.removeItem('da_terrain_access');
  localStorage.removeItem('da_terrain_refresh');
  localStorage.removeItem('da_terrain_user');
}
