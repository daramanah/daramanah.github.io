...
function getCurrentRoute() {
  const hash = window.location.hash || '#/missions';
...
function renderLayout(contentHtml) {
  ...
  <aside class="sidebar hidden md:flex fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 flex-col z-40">
    <div class="h-16 flex items-center px-6 border-b border-gray-100">
      <a href="/terrain/" class="text-xl font-serif font-bold text-brand-navy">DarAmanah</a>
      <span class="text-[10px] bg-teal-600 text-white px-1.5 py-0.5 rounded-full ml-1.5">Terrain</span>
    </div>
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">${sidebarLinks}</nav>
    <div class="p-4 border-t border-gray-100">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 bg-teal-600 text-white rounded-full flex items-center justify-center text-xs font-bold">${escapeHtml(initials)}</div>
  ...
  <header class="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 z-40">
    <div class="flex items-center gap-1.5">
      ...
      <span class="text-[9px] bg-teal-600 text-white px-1 py-0.5 rounded-full">Terrain</span>
    </div>
