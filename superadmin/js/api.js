// DarAmanah Super Admin — API fetch wrapper
// Uses distinct localStorage keys (da_super_*) to avoid colliding with the
// antenne SPA when both are opened in the same browser.
const API_BASE = 'https://api.daramanah.family';

function getTokens() {
  return { access: localStorage.getItem('da_super_access'), refresh: localStorage.getItem('da_super_refresh') };
}
function setTokens(access, refresh) {
  if (access) localStorage.setItem('da_super_access', access);
  if (refresh) localStorage.setItem('da_super_refresh', refresh);
}
function clearTokens() {
  localStorage.removeItem('da_super_access');
  localStorage.removeItem('da_super_refresh');
  localStorage.removeItem('da_super_user');
}
