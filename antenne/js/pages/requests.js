...
function renderList(status) {
  ...
  if (filtered.length === 0) { document.getElementById('req-empty').classList.remove('hidden'); document.getElementById('req-list').classList.add('hidden'); }
  else { document.getElementById('req-empty').classList.add('hidden'); const list = document.getElementById('req-list'); list.innerHTML = filtered.map(renderCard).join(''); list.classList.remove('hidden'); }
}
